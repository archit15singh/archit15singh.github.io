...
description: "An agent skill that turns a terse pointer into a frame and a top-k you would actually work. Two files, no runtime. The stop is a four-check, not a feeling."
...
**1. It is taken on the whole field.** A cut from a partial view is premature, a conclusion dressed as analysis. So the first move ranks nothing: one extra sibling pass, one snowball, searching the *job* not the brand. [Zwicky's morphological box](https://www.swemorph.com/ma.html). Wohlin's snowballing guidelines.

**2. It keeps the few and names the many.** Keep the non-dominated set on the load-bearing columns. Name the tail once, leave it. [Juran 1974](https://www.juran.com/wp-content/uploads/2021/03/The-Non-Pareto-Principle-1974.pdf). The front is what you commit to.
...
**Out.** A frame you can reuse, a top-k taken from the front, and a written answer to "would you start item 1 tomorrow?" The nine emit slots live in `SKILL.md`. The post does not restate them.
...
The pointer sounds like a product. The object is not. After naming, it is [anthropics/defending-code-reference-harness](https://github.com/anthropics/defending-code-reference-harness), and the class it sits in: a closed-loop cyber reasoning system. Job: autonomous recon → find → verify → patch, with a witness that is not a paragraph.
...
That question sorts the front. Three genera survive: closed-loop CRS, a skill pipeline on a coding agent, a whitebox AppSec hunter. Everything else is the tail, named once.
...
The frame I imported is Stanford Symbolic Systems: philosophical analysis, formal methods, computational methods, empirical cognitive science. [Sourced](https://symsys.stanford.edu/undergraduates/major-policies-requirements/core-requirements), not invented that afternoon. Under that frame a CRS is a hybrid: neural finder, symbolic grader, public language, second agent forbidden from the first agent's inner story.
...
| | Claim | What it does |
|---|---|---|
| H1 Formalization | Next leap is proof-like witnesses and fix-gates | Promotes the proof object |
| H2 Communication | Bottleneck is symbols a human can merge | Promotes the merge artifact |
| H3 Architecture | Load-bearing invention is the trust split | Promotes bytes-only |
| H4 CogSci of the researcher | Model the human process | Traces, de-biasing. Measurement, not the product |
| H5 Hybrid is the object | Neural finder + symbolic grader *is* the system | The frame |
| H6 Philosophy-first is vocabulary | Names that do not ship | Demote to vocabulary |
...
The full card is [examples/worked-example.md](https://github.com/archit15singh/vital-few/blob/main/examples/worked-example.md).
